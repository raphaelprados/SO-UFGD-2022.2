...
void computarSubmatriz(void* parametros_funcao) {
    ...
    while (submatrizes_verificadas != submatrizes.size()) {
        ...
        if (submatriz.regular) { // Submatrizes regulares
            ...
            long tamanho_submatriz = submatriz.tamanho;
            int j = 0;
            for (int i = submatriz.top_left.x; i != fim_matriz.x + 1 || tamanho_submatriz != 0; i++) {
                j = (i == submatriz.top_left.x ? submatriz.top_left.y : 0);
                for (; j < fim_matriz.y + 1 && tamanho_submatriz != 0; j++) {
                    if (primality_test(matriz[i][j]))
                        thread_param->primes++;
                    tamanho_submatriz--;
                }
